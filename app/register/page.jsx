...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const pesanError = err.response?.data?.error || "Pendaftaran gagal, cek koneksi server.";
      setError(`❌ ${pesanError}`);
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      <div className="login-shell">
        <div className="login-left">
          ...
          <div className="login-tagline">Bergabung bersama <span>Pejuang Rupiah</span> NTT</div>
          <p className="login-desc">Daftarkan diri Anda untuk mempermudah proses penukaran uang tidak layak edar melalui layanan resmi BI.</p>
        </div>
            ...
            <form onSubmit={handleRegister}>
              ...
              <div className="form-row">
                <div className="form-group" style={{ position: 'relative' }}>
                  ...
                  <span onClick={() => setShowSandi(!showSandi)} style={{ position: 'absolute', right: '12px', bottom: '10px', cursor: 'pointer' }}>
                    {showSandi ? '🙈' : '👁️'}
                  </span>
                </div>
                <div className="form-group" style={{ position: 'relative' }}>
                  ...
                  <span onClick={() => setShowKonfirmasi(!showKonfirmasi)} style={{ position: 'absolute', right: '12px', bottom: '10px', cursor: 'pointer' }}>
                    {showKonfirmasi ? '🙈' : '👁️'}
                  </span>
                </div>
              </div>

              <button type="submit" className="btn-full" style={{ marginTop: '10px' }} disabled={loading}>
